...
class Project(AuditMixin, CommonMixin, db.Model):
    ...
    trial_phase_id: Mapped[int] = mapped_column(ForeignKey(TrialPhase.id), index=True, nullable=False)
    ...
    main_funding_dhsc_nihr_funding_id: Mapped[int] = mapped_column(ForeignKey(MainFundingDhscNihrFunding.id), index=True, nullable=False)
    ...
    main_funding_industry_id: Mapped[int] = mapped_column(ForeignKey(MainFundingIndustry.id), index=True, nullable=False)
