...
class Project(AuditMixin, CommonMixin, db.Model):
    ...
    local_rec_number: Mapped[str] = mapped_column(String(50), nullable=True)
    iras_number: Mapped[str] = mapped_column(String(50), nullable=True)
    cpms_id: Mapped[str] = mapped_column(String(50), nullable=True)
    main_funding_source: Mapped[int] = mapped_column(String(500), nullable=False)

    start_date: Mapped[date] = mapped_column(nullable=True, index=True)
    end_date: Mapped[date] = mapped_column(index=True, nullable=True)
    participants_recruited_to_centre_fy: Mapped[int] = mapped_column(nullable=True)
    ...
    nihr_priority_area_id: Mapped[int] = mapped_column(ForeignKey(NihrPriorityArea.id), index=True, nullable=True)
